...
// 7 by 7 matrix representing the finder pattern found in each corner
extern const unsigned char pattern_finder[7][7];

// 5 by 5 matrix representing the alignment pattern
extern const unsigned char pattern_alignment[5][5];

// 45-byte long character set for the alphanumeric encoding mode
extern const char* charset_alpha;

// version to version range translation
extern const unsigned char version_range[41];

// Error Correction Level is handled as L..H = 0..3 but they are coded as 1, 0, 3, 2
...
// position of alignment patterns for each version the v-th row contains the list of coordinates the positions are all combinations of this coordinates except for the three corner (finder pattern overlaps)
